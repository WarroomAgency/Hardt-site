...
**Photography — largely resolved, August 2026.** Peter's own photos now
fill the portrait, both team headshots, the before/after slider and one
project card. Three project cards and the hero still run on illustration
or free stock, blocked on watermark-free files (see PHOTO-BRIEF).

**Photography — the original gap.** Every image on the site is an original vector
illustration generated by `tools/make_images.py`. They're art-directed to the
brand and they hold up, but they are not photographs and nobody will mistake
them for photographs.

I can't download stock imagery — web fetching in this environment is restricted
to the approved tools, and those don't pull binaries. So the options are:

1. **Peter's own photos.** He said he has before/during/after of all five
   projects. That's the best outcome by a distance and costs nothing.
2. **Licensed stock**, picked by hand. Search terms that fit the brand:
   *"california ranch house exterior golden hour"*, *"stucco bungalow front
   door"*, *"contractor renovation interior daylight"*. Avoid anything with a
   handshake, a sold sign, or a smiling couple holding keys — that's the
   category cliché the brand is defined against.
3. **A half-day shoot.** Founder portrait plus one finished project. This is
   what the brand book actually calls for.

Drop files into `assets/img/` using the existing names and they appear
everywhere automatically. Delete `tools/make_images.py` once they all land.
